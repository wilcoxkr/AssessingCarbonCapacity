...
{######### Set starting C pools ##############################################
  ### Hays C pools: 
  ### X1foliage - 110.25 g C/m2 -- this is standing aboveground biomass from 2014 (because this is the year I have belowground biomass for) 245 x 0.45 = 110.25
  ### X2fineroots - 239 g C/m2 -- approximate fine+coarse root sampled to 15 cm from Derner et al. 2009: 531 x .45 = 144.1
  ### X3litter - 138.2 g C m-2 -- Pdead (aboveground) + bnpp measurement averages 2013-2014 (assuming constant SC root this is the biomass turning over so (173.3(Pdead) + 136.8(BNPP)) x .45 = 138.2
  ### X4 microbial C - 99.5 g C m-2 -- Couldn't find anything in the literature so I'll take the average between Konza and SGS so... avg(166, 32.9) = 99.5 g m-2
  ### X5 Slow SOM -- 1728.6 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2865.2-99.5*.625=1728.6
}### X6 Passive SOM -- 1037.1 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2865.2-99.5*.375=1037.1

x0 =		c(110.25,239,138.2,99.5,1728.6,1037.1) # Initial values of C pools
...
{######## Input matrix -- The proportion of C from Phs to above versus belowground pools... Using EDGE knz data and data from Wilcox et al. 2015 GCB to estimate average BNPP:ANPP -- mean(.55, .55, 0.50, 1.0) = 0.65
  ### So, root:shoot=.40 meaning proportions of C aboveground is 1/1.40=0.71 and belowground is 0.40/1.40=0.29
  # aboveground gets 71% biomass C and belowground gets 29% -- Aboveground: 0.55*.71= .39, belowground: .55*0.29=0.16
  b = c(0.39,0.16,0,0,0,0) # Took .55 which is what Xu et al 2006 put as total Phs going to biomass (I could always look this up for Konza) and divided it up based on root:shoot ratio from the site - 0.65 for sev
  # Read in Carbon (from phs) data set --  % u1 is C input in ambient, u2 is C in elevated #
  cbn = read.csv( "hys_gpp_daily_13-15.csv") # Run climate forcing data through TECO to get gpp -- having trouble getting it to match up quite right due to very high production values in 2013... #
  u = cbn$gpp
}

{######## The following code is from the temp_moist subroutine ##############	
  # Read in and set soil moisture data #
  moist_1 <- read.csv("hys_smoist_tau_13-15.csv")
  moist <- moist_1$Smoist
  # Read in and set temperature data #
  temp_1 <- read.csv("hys_stemp_tau_13-15.csv")
  temp <- temp_1$temp
...
{######## Read in data sets #################################################
  soil_respiration <- read.csv("hys_co2flux_13-15.csv")
  ...
  SCabove <- read.csv("hys_SCabove.csv")
  SCaboveTime <- SCabove$day
  SCaboveValue <- SCabove$stcrop *0.45## Convert to g C m-2
  SCbelow <- read.csv("hys_SCbelow_fromDerner2009.csv") # From Derner 2009 Plant and Soil
  SCbelowTime <- SCbelow$day
  SCbelowValue <- SCbelow$stcrop *0.45 ## Convert to g C m-2
  Litter <- read.csv("hys_Litter.csv")
  LitterTime <- Litter$day
  LitterValue <- Litter$pdead *0.45 ## Convert to g C m-2
  
  # Soil carbon # 
  soilMineralCarbonTime <- 725 # EDGE hys measurement in 2012... assuming it's stable
  mineralCbnValue <- 2865.2 # EDGE hys measurement ... assuming it's stable
}
...
{######## Mappings of which C pools (from model) make up each value given by each data set ####################
  ...
  phi_litter 			<- c(0,			0,			0.71*1,		0,			0,			0)	## maps 71% of litter C to litter measurement since model litter incorporates both above and belowground litter			
  cdif  <- (cmax-cmin)
  fdif <- fmax-fmin
...
write.csv(c_4write, file="hys_c params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(f_4write, file="hys_f params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(mscut_4write, file="hys_mscut params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(Q10_4write, file="hys_Q10 params_360000 4chains_13-15.csv", row.names=FALSE)
