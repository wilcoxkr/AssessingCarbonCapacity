...
## but I don't know that it necessarily translates, the rest I put towards structural litter
	...
	AC[3,] <- c(	0,  	 c_new[2],    -c_new[3],      	  0,                   0,                0) # I'm not sure that all of c2 should go into litter... if plant respiration is occurring, this should detract from the amount of C in this pool... and since we put GPP into the model, this should be accounted for... ask Yiqi Zheng about this.
...
### X1foliage - 210.6 g C/m2 -- this is standing aboveground biomass for 2014 (because this is the same year I have belowground biomass for) 468 x 0.45 = 210.6
### X2fineroots - 466.7 g C/m2 -- this is standing crop root biomass from my irrigation transect study (in review) 1037.2 x .45 = 466.7
### X3litter - 123.2 g C m-2 -- Only belowground since plots are burned annually -- Using BNPP value and assuming constant SC root biomass, we get a general turnover value x 273.7 x .45 = 123.2
### X4 microbial C - 166.4 g C m-2 -- ### CHY measurement is 40.1 g/m2 (Ingram et al. 2008 - Grazing impacts on soil carbon)
### X5 Slow SOM -- 1730.7 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2935.5-166.4*.625=1730.7
### X6 Passive SOM -- 1038.4 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2935.5-166.4*.375=1038.4
}

x0 =		c(210.6,466.7,123.2,166.4,1730.7,1038.4) # Initial values of C pools
...
{######## Input matrix -- The proportion of C from Phs to above versus belowground pools... Using EDGE knz data and data from Wilcox et al. 2015 GCB to estimate average BNPP:ANPP -- mean(.55, .55, 0.50, 1.0) = 0.65
	### So, root:shoot=0.63 meaning proportions of C aboveground is 1/1.63=0.61 and belowground is 0.63/1.63=0.39
# aboveground gets 61% biomass C and belowground gets 39% -- Aboveground: 0.55*.61= .39, belowground: .55*0.39=0.16
b = c(0.34,0.21,0,0,0,0) # Took .55 which is what Xu et al 2006 put as total Phs going to biomass (I could always look this up for Konza) and divided it up based on root:shoot ratio from the site - 0.65 for sev
# Read in Carbon (from phs) data set --  % u1 is C input in ambient, u2 is C in elevated #
cbn = read.csv( "knz_gpp_daily_13-15_v2.csv") # Run climate forcing data through TECO to get gpp -- having trouble getting it to match up quite right due to very high production values in 2013... #
u = cbn$gpp
}

{######## The following code is from the temp_moist subroutine ##############	
# Read in and set soil moisture data #
moist_1 <- read.csv("knz_smoist_tau_13-15.csv")
moist <- moist_1$Smoist
# Read in and set (soil?) temperature data #
temp_1 <- read.csv("knz_stemp_tau_13-15.csv")
temp <- temp_1$temp
...
{######## Read in data sets #################################################
soil_respiration <- read.csv("knz_co2flux_13-15_corrected co2 and cleaned.csv")
...
SCabove <- read.csv("knz_SCabove.csv")
SCaboveTime <- SCabove$day
SCaboveValue <- SCabove$stcrop *0.45## Convert to g C m-2
SCbelow <- read.csv("knz_SCbelow_from IrrT study.csv") # Only one measurement, in 2013 from irrt bnpp study
SCbelowTime <- SCbelow$day
SCbelowValue <- SCbelow$stcrop *0.45 ## Convert to g C m-2

# Soil carbon # 
soilMineralCarbonTime <- 725 # EDGE KNZ measurement in 2012... assuming it's stable
	mineralCbnValue <- 2935.5 # EDGE KNZ measurement ... assuming it's stable
}
...
write.csv(c_4write, file="knz_c params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(f_4write, file="knz_f params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(mscut_4write, file="knz_mscut params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(Q10_4write, file="knz_Q10 params_360000 4chains_13-15.csv", row.names=FALSE)
