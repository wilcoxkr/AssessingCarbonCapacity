...
{######### Set starting C pools ##############################################
  ...
  ### X2fineroots - 32.7 g/m2 -- this is standing crop root biomass from 2014 = 72.62 * 0.45 =  32.7 gC/m2
...
x0 =		c(22.7,32.7,41.0,64.5,631.7,379.0) # Initial values of C pools
...
{######## Input matrix -- The proportion of C from Phs to above versus belowground pools... Using EDGE knz data and data from Wilcox et al. 2015 GCB to estimate average BNPP:ANPP -- mean(.55, .55, 0.50, 1.0) = 0.65
  ### So, root:shoot=2.03 meaning proportions of C aboveground is 1/3.03=0.33 and belowground is 2.03/3.03=0.67
  # aboveground gets 71% biomass C and belowground gets 29% -- Aboveground: 0.55*.33= .18, belowground: .55*0.67=0.37
  ...
  cbn = read.csv( "gpp_daily_blue_modbyyr.csv") # Run climate forcing data through TECO to get gpp -- having trouble getting it to match up quite right due to very high production values in 2013... #
  u = cbn$gpp
}

{######## The following code is from the temp_moist subroutine ##############	
  # Read in and set soil moisture data #
  moist_1 <- read.csv("moist5cm_blue_sev13_14_fromSite_tao.csv")
  moist <- moist_1$Smoist
  # Read in and set soil temperature data #
  temp_1 <- read.csv("temp5cm_blue_sev13_14_fromSite_tao.csv") 
  temp <- temp_1$Stemp
...
{######## Read in data sets #################################################
  soil_respiration <- read.csv("soil_resp_blue.csv")
  ...
  mineralCbnValue <- 1159.2 # sev measurement ... assuming it's stable
}
...
write.csv(c_4write, file="sevblu_c params_360000 4chains_13-15_run3.csv", row.names=FALSE)
...
write.csv(f_4write, file="sevblu_f params_360000 4chains_13-15_run3.csv", row.names=FALSE)
...
write.csv(mscut_4write, file="sevblu_mscut params_360000 4chains_13-15_run3.csv", row.names=FALSE)
...
write.csv(Q10_4write, file="sevblu_Q10 params_360000 4chains_13-15_run3.csv", row.names=FALSE)
