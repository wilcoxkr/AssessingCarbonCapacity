...
#set.seed(seed.1)
setwd( "C:\\Users\\Kevin.Wilcox\\Desktop\\fromOldComp\\FULL_ANALYSES_July2017\\DA input\\sgs\\")
...
{######### Set starting C pools ##############################################
  ### SGS C pools: 
  ### X1foliage - 81.0 g C/m2 -- this is standing aboveground biomass (no Pdead) from 2014 (because this is the year I have belowground biomass for) 180.0 x 0.45 = 81.0
  ### X2fineroots - 396.4 g C/m2 -- standing crop root biomass average SGS data_see Milchunas 2009 880.9 x .45 = 271.6
  ### X3litter - 107.9 g C m-2 -- Pdead (aboveground - 2014) + bnpp measurement from 2014 (assuming constant SC root this is the biomass turning over so (21.7(Pdead) + 217.98(BNPP)) x .45 = 96.5
  ### X4 microbial C - 32.9 g/m2 (Elliot et al. 1984 - Mineralization dynamics in fallow dryland)
  ### X5 Slow SOM -- 766.9 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 1260.0-32.9*.625=766.9
}### X6 Passive SOM -- 460.2 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 1260.0-32.9*.375=460.2

x0 =		c(81.0,396.4,107.9,32.9,766.9,420.2) # Initial values of C pools
...
{######## Input matrix -- The proportion of C from Phs to above versus belowground pools... Using EDGE knz data and data from Wilcox et al. 2015 GCB to estimate average BNPP:ANPP -- mean(.55, .55, 0.50, 1.0) = 0.65
  ### So, root:shoot=1.2 meaning proportions of C aboveground is 1/2.2=0.45 and belowground is 1.2/2.2=0.55
  # aboveground gets 45% biomass C and belowground gets 55% -- Aboveground: 0.55*.45= .25, belowground: .55*0.55=0.30
  b = c(0.25,0.30,0,0,0,0) # Took .55 which is what Xu et al 2006 put as total Phs going to biomass (I could always look this up for Konza) and divided it up based on root:shoot ratio from the site - 0.65 for sev
  # Read in Carbon (from phs) data set --  % u1 is C input in ambient, u2 is C in elevated #
  cbn = read.csv( "sgs_gpp_daily_13-15.csv") # Run climate forcing data through TECO to get gpp -- having trouble getting it to match up quite right due to very high production values in 2013... #
  u = cbn$gpp
}

{######## The following code is from the temp_moist subroutine ##############	
  # Read in and set soil moisture data #
  moist_1 <- read.csv("sgs_smoist_tau_13-15.csv")
  moist <- moist_1$Smoist
  # Read in and set soil temperature data #
  temp_1 <- read.csv("sgs_stemp_tau_13-15.csv") 
  temp <- temp_1$temp
...
{######## Read in data sets #################################################
  soil_respiration <- read.csv("sgs_co2flux_13-15.csv")
  soil_respiration <- soil_respiration[!is.na(soil_respiration$co2.flux),] 
  soilTime <- soil_respiration$day
  soilResValue <- soil_respiration$co2.flux
  SCabove <- read.csv("sgs_SCabove.csv")
  SCaboveTime <- SCabove$day
  SCaboveValue <- SCabove$stcrop *0.45## Convert to g C m-2
  SCbelow <- read.csv("sgs_SCbelow_fromSGSltdata.csv") # 
  SCbelowTime <- SCbelow$day
  SCbelowValue <- SCbelow$stcrop *0.45 ## Convert to g C m-2
  Litter <- read.csv("sgs_Litter.csv")
  LitterTime <- Litter$day
  LitterValue <- Litter$pdead *0.45 ## Convert to g C m-2
  
  # Soil carbon # 
  soilMineralCarbonTime <- 725 # EDGE sgs measurement in 2012... assuming it's stable
  mineralCbnValue <- 1260.0 # EDGE sgs measurement ... assuming it's stable
}
...
{######## Mappings of which C pools (from model) make up each value given by each data set ####################
  ...
  phi_litter 			<- c(0,			0,			0.45*1,		0,			0,			0)			# 45% of NPP is ANPP
  cdif  <- (cmax-cmin)
  fdif <- fmax-fmin
...
{######## Simulation starts #################################################
  ...
  for(chain in 1:nchains){
    ...
    for(simu in 1:nsim){
      ...
      Litter_simu <- numeric(length(LitterTime))
      ...
      for(i in 1:length(LitterTime)){
        Litter_simu[i] <- phi_litter %*% x[,LitterTime[i]]
      }
      ...
      J[5]  =   (norm(as.matrix(Litter_simu-LitterValue),"f"))^2		
      
      J_new <- (J[1]/DJ1+J[2]/DJ2+J[3]/DJ3+J[4]/DJ4+J[5]/DJ5)
...
write.csv(c_4write, file=paste0("sgs_c params_",nsim," sims_",nchains," chains_",Sys.Date(),".csv"), row.names=FALSE)
...
write.csv(f_4write, file=paste0("sgs_f params_",nsim," sims_",nchains," chains_",Sys.Date(),".csv"), row.names=FALSE)
...
write.csv(mscut_4write, file=paste0("sgs_mscut params_",nsim," sims_",nchains," chains_",Sys.Date(),".csv"), row.names=FALSE)
...
write.csv(Q10_4write, file=paste0("sgs_Q10 params_",nsim," sims_",nchains," chains_",Sys.Date(),".csv"), row.names=FALSE)
