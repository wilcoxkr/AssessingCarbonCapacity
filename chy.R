...
{######### Set starting C pools ##############################################
  ### CHY C pools: 
  ### X1foliage - 206.9 g C/m2 -- this is standing aboveground biomass from 2014 (because this is the year I have belowground biomass for) 459.7 x 0.45 = 110.25
  ### X2fineroots - 297.3 g C/m2 -- From Redente 1989: 660.7 x .45 = 297.3
  ### X3litter - 144.8 g C m-2 -- Pdead (aboveground) + bnpp measurement averages 2013-2014 (assuming constant SC root this is the biomass turning over so (34.7(Pdead) + 287.1(BNPP)) x .45 = 138.2
  ### X4 microbial C - 40.1 g C m-2 -- CHY: From Balesdent 1987
  ### X5 Slow SOM -- 1494.2 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2430.8-40.1*.625=1494.2
}### X6 Passive SOM -- 896.5 g C m-2 -- using proportions from Parton et al. 1988 for slow (0.625) and passive C (0.375) applied to EDGE soil C data - microbial C, so: 2430.8-40.1*.375=896.5

x0 =		c(206.9,297.3,144.8,40.1,1494.2,896.5) # Initial values of C pools
...
{######## Input matrix -- The proportion of C from Phs to above versus belowground pools... Using EDGE knz data and data from Wilcox et al. 2015 GCB to estimate average BNPP:ANPP -- mean(.55, .55, 0.50, 1.0) = 0.65
  ### So, root:shoot=1.7 meaning proportion of C aboveground is 1/2.7=0.37 and belowground is 1.7/2.70=0.63
  # aboveground gets 37% biomass C and belowground gets 63% -- Aboveground: 0.55*.37= .20, belowground: .55*0.63=0.35
  b = c(0.20,0.35,0,0,0,0) # Took .55 which is what Xu et al 2006 put as total Phs going to biomass (I could always look this up for Konza) and divided it up based on root:shoot ratio from the site - 0.65 for sev
  # Read in Carbon (from phs) data set --  % u1 is C input in ambient, u2 is C in elevated #
  cbn = read.csv( "chy_gpp_daily_13-15.csv") # Run climate forcing data through TECO to get gpp -- having trouble getting it to match up quite right due to very high production values in 2013... #
  u = cbn$gpp
}

{######## The following code is from the temp_moist subroutine ##############	
  # Read in and set soil moisture data #
  moist_1 <- read.csv("chy_smoist_tau_13-15.csv")
  moist <- moist_1$Smoist
  # Read in and set soil temperature data #
  temp_1 <- read.csv("chy_stemp_tau_13-15.csv") 
  temp <- temp_1$temp
...
{######## Read in data sets #################################################
  soil_respiration <- read.csv("chy_co2flux_13-15.csv")
  ...
  mineralCbnValue <- 2430.8 # EDGE hys measurement ... assuming it's stable
}
...
{######## Mappings of which C pools (from model) make up each value given by each data set ####################
  ...
  phi_litter 			<- c(0,			0,			0.36*1,		0,			0,			0)	## maps 36% of litter C to litter measurement since model litter incorporates both above and belowground litter		
  cdif  <- (cmax-cmin)
  fdif <- fmax-fmin
...
write.csv(c_4write, file="chy_c params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(f_4write, file="chy_f params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(mscut_4write, file="chy_mscut params_360000 4chains_13-15.csv", row.names=FALSE)
...
write.csv(Q10_4write, file="chy_Q10 params_360000 4chains_13-15.csv", row.names=FALSE)
